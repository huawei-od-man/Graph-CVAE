...
class GraphVAE(nn.Module):
    ...
    def generate(self, num_samples=1):
        ...
        with torch.no_grad():
            ...
            generated_graphs = []
            for adj_params in adj_pred:
                # Convert lower triangular params to full adjacency matrix
                adj_lower = self.recover_adj_lower(adj_params.cpu())
                adj_full = self.recover_full_adj_from_lower(adj_lower)
                # Binarize using 0.5 threshold
                adj_binary = (adj_full > 0.5).float()
                generated_graphs.append(adj_binary.numpy())
